...
- **Экономия действий — отдельная обязательная линза (скорость работы).** Оценивай не только «есть ли
  ...
  которую трогаешь (создаёшь или редактируешь), быстро посчитай шаги до результата для 2–3 топ-сценариев и
  **проактивно предложи ускорения, не дожидаясь просьбы**: одно действие вместо трёх, drag-and-drop и
  тап-цели, разумные дефолты, что показать вверху списка/сортировка под задачу, batch-операции, клавиатура,
  Undo вместо подтверждений. При дизайне/ревью/редизайне выноси наверх короткий список «ускорений» (выигрыш →
  усилие), а не только «чего не хватает». Не жди, что пользователь сам придумает микро-улучшения интерфейса —
  это твоя работа как продуктового дизайнера. (Баланс прежний: не над-инженерить и не плодить лишнее —
  предлагай дешёвые высокорычажные ускорения, а не украшательство.)
- **Обратная связь во время жеста — часть этой линзы.** Мало сократить шаги — на каждом интерактивном
  действии должно быть видно, ЧТО произойдёт: hover/drag-over/active-состояния, подсветка валидной цели
  («на кого/куда упадёт, если отпустить»), приподнятая выбранная сущность, индикатор места вставки. Любой
  drag-and-drop/выбор-цель без подсветки цели — незаконченная фича. Делая обратную связь для ОДНОГО способа
  ввода (клик), сделай её и для ДРУГОГО (перетаскивание) — они должны быть согласованы, не «у клика есть, у
  drag нет».
...
**Любое изменение проверяй сам, прежде чем считать его готовым и докладывать «работает».
Не предполагай — проверяй.** Гипотеза о причине/результате не равна факту; «должно работать» —
не «работает». Этот раздел приоритетен наравне с правилом коммита: незавершённая проверка =
незавершённая задача.

- **Минимум для любого изменения кода/конфига:** `dotnet build` (или `--no-incremental`/удаление `obj/`
  для Razor/Blazor — см. граблю #16), `dotnet test`, `dotnet format`. Не покрыто тестами — добавить тесты.
- **Изменение видимого/рантайм-поведения проверяй на РЕАЛЬНОМ пути пользователя, а не только сборкой.**
  Визуальное (вёрстка, доска, страница) — поднять свежий билд и снять скриншот через headless Playwright
  (см. [e2e](e2e/), граблю #17 и память про мобильный харнесс); поведение API/игры — дёрнуть реальный
  эндпоинт/хаб или интеграционный тест. CSS, стили, скрипты тонких клиентов на глаз по диффу НЕ проверяются.
- **Запущенный чужой инстанс (Rider/AppHost) не отражает твой свежий билд** — останови и подними свой
  (хот-релоад не ловит правки). Свои тестовые процессы за собой гаси, временные пробники удаляй.
- **Проверяй ту же конфигурацию, что у пользователя.** Холодная сборка ≠ инкрементальная; прод-путь ≠
  dev-заглушка. Если баг воспроизводится только «вхолодную» — воспроизводи вхолодную.
- **Докладывай честно по факту проверки:** что именно проверил и каким способом (билд/тесты/скриншот/лог).
  Не выдавай непроверенное за проверенное; если проверить не смог — скажи прямо, что осталось проверить.
