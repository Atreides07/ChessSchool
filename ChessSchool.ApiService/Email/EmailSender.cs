...
namespace ChessSchool.ApiService.Email;

/// <summary>Отправитель писем доменного API (сейчас — прогресс ученика родителю).</summary>
public interface IEmailSender
{
    Task SendAsync(string to, string subject, string htmlBody, CancellationToken ct = default);
}

/// <summary>Конфиг почты (секция <c>Email</c>). Dev — mailpit (host/port, без auth/TLS); прод — реальный SMTP.
/// Тот же формат, что у IdP — локально AppHost прокидывает mailpit, в проде задаётся реальный SMTP.</summary>
public sealed class EmailOptions
{
    public string From { get; set; } = "ChessSchool <no-reply@chessschool.local>";
    ...
    public static EmailOptions FromConfig(IConfiguration config)
    {
        ...
        {
            From = s["From"] ?? "ChessSchool <no-reply@chessschool.local>",
            Host = s["Smtp:Host"],
...
/// <summary>Фолбэк без SMTP (dev без mailpit / тесты): пишет письмо в лог. Реальную отправку НЕ выполняет.</summary>
