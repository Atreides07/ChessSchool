...
- **MFA (TOTP)** ([Totp.cs](ChessSchool.Auth/Totp.cs)/[MfaService.cs](ChessSchool.Auth/MfaService.cs)): опциональная
  ...
  коды одноразовые (в БД только хэш). Настройка — `/account/mfa`; миграция `AddMfa`. **Обязательна для админов**
  (`Auth:Mfa:RequiredForAdmins`, дефолт вкл): админ без 2FA форсится в настройку на входе, а authorize не выдаёт
  код до включения (нет `role=admin` в приложении без второго фактора); отключить 2FA админу нельзя.
...
18. **Фингерпринт `@Assets` ломает совпадение по литеральному имени файла.** Симптом: e2e
    ([broadcasts.spec.js](e2e/tests/broadcasts.spec.js)) упал `expect(scripts).toContain('broadcast.js')` —
    в проде/при `MapStaticAssets` скрипт отдаётся как `broadcast.<hash>.js` (через `@Assets["js/broadcast.js"]`
    в [App.razor](ChessSchool.Arena/Components/App.razor)), а не как `broadcast.js`. Причина — задокументированное
    прод-поведение фингерпринта (архитектурное решение про `@Assets`), но тест матчил точное имя. Лечение:
...
22. **Тонкий клиент: часы/анимацию обновлять В МЕСТЕ; НЕ пересоздавать узел и НЕ вести общий отсчёт от `stateAt` на каждый пуш.**
    Симптом (арена, [tournament.js](ChessSchool.Arena/wwwroot/js/tournament.js)): во время своей партии часы «дёргаются» и
    перерисовываются с лагом, доска будто обновляется, «как будто соперник уже сходил». Причина двойная: (1) Арена шлёт
    пуш на ЛЮБОЕ событие турнира (ход бота в чужой доске), а `onShared` на каждый такой пуш делал `stateAt = Date.now()`,
    при этом `state.myGame` (с часами) обновлялся лишь троттлингом раз в ~700мс → `tickClocks` (`base - (now-stateAt)`)
    постоянно откатывал мои часы к устаревшему `base` → отсчёт «замирал» и прыгал раз в 700мс (это и есть «лаг»); (2)
    `updateMyGameCard` на каждый пуш делал `players-top/bottom.innerHTML = playerRow(...)`, **пересоздавая узел `.js-clock`** →
    сброс текста до серверного значения, перезапуск CSS active-клетки (мерцание «как ход») и reflow строки над/под доской
    (доска дёргается). Лечение: (1) у каждого `.js-clock` своя база отсчёта `data-at` — часы моей партии несут `myGameAt`
    (момент персонального обновления), а не общий `stateAt`, который сбрасывают чужие пуши; чужие мини-доски берут `stateAt`
    (их ms свежие в каждом пуше). (2) `updatePlayerRow` правит часы В МЕСТЕ (`data-ms/at/active`, toggle `.active`), а
    `innerHTML` строки пересобирает только при смене структурной подписи (имя/берсерк/бот/флаг); контролы — по подписи html.
    Правило для тонких клиентов: частые пуши обновляют существующие узлы точечно; «база времени» — на самом узле, а не одна
    глобальная на все часы. Регрессия — [clock-smoothness.spec.js](e2e/tests/clock-smoothness.spec.js) (отсчёт от `data-at`).
    Доп. симптом «часы переключаются на соперника с лагом»: свой ход рисовался оптимистично (`renderBoard`), а активные
    часы ждали серверный `applyState` (round-trip). Лечение — `switchClocksOptimistic()` в `commitMove`: сразу после хода
...
25. **OIDC-колбэк `/signin-oidc`: сбой удалённого входа («Correlation failed») обязан деградировать, а не падать 500.**
    ...
    **израсходовала** correlation-cookie той сессии. Клик по письму повторно проигрывает тот authorize → код
    ...
    ([SsoExtensions](ChessSchool.WebAuth/SsoExtensions.cs)): `OnRemoteFailure` гасит исключение — но простого
    редиректа на `/` НЕДОСТАТОЧНО (см. подпункт ниже). Правило: OIDC-хендлеры ВСЕГДА обрабатывают
    `OnRemoteFailure`; кросс-сервисные auth-раунды (ссылка из письма → OIDC-колбэк приложения) проверять
    сквозным прогоном на реальном приложении, а не по существующей сессии.
    **Подвох (наступили следом): редирект `OnRemoteFailure` на «/» ЛЕЧИТ 500, но НЕ обновляет claim'ы приложения.**
    Симптом: после регистрации И подтверждения (клик по ссылке из письма) баннер «подтвердите e-mail» всё равно
    висит, платные действия закрыты. Причина: confirm на IdP ставит `EmailConfirmed=true` и обновляет cookie IdP,
    но старый authorize-URL из письма несёт израсходованный correlation → callback `/signin-oidc` падает →
    `OnRemoteFailure` уводит на «/» → **cookie ПРИЛОЖЕНИЯ не переиздаётся** и хранит `email_verified=false`
    (authorize/userinfo читают `EmailConfirmed` из БД свежим — [OidcEndpoints](ChessSchool.Auth/OidcEndpoints.cs) —
    поэтому достаточно заново пройти OIDC, но редирект на «/» этого не делает). Лечение: `OnRemoteFailure` делает
    **свежий `ChallengeAsync`** (новый correlation) — сессия IdP уже валидна → повторный вход проходит прозрачно и
    приложение получает cookie с `email_verified=true`; баннер исчезает. One-shot guard-cookie `oidc_retry`
    (SameSite=None, чтобы дожить до кросс-сайтового callback; снимается в `OnTokenValidated` при успехе) рвёт петлю:
    если и свежий callback падает — второй раз не пытаемся, уводим на «/». Правило: сбой OIDC-callback на пути
    «подтверждение из письма» лечится ПЕРЕ-АВТОРИЗАЦИЕЙ (fresh challenge), а не просто редиректом; claim'ы приложения
    обновляются только новым OIDC-раундом. Проверять сквозным прогоном: регистрация → клик по ссылке из письма →
