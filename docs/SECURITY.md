...
| Метрики для алертинга | ✅ | Счётчик `chessschool.auth.events` с тегом типа ([AuthMetrics](../ChessSchool.Auth/AuthMetrics.cs)) экспортируется через OpenTelemetry — на нём в проде строятся дашборды и пороговые алерты (всплеск `LoginFailure`/`NewDeviceLogin`). |
...
| Готовые пороговые алерты/дашборды | ⏳ | Метрики и события есть; конкретные правила алертинга настраиваются в системе мониторинга прод-окружения (вне кода). |
