...
4. **Rate-limit на письмо прогресса родителю** (`POST /students/{id}/send-progress`, ApiService) — сейчас без лимитера. 🟡 Остаточный риск низкий: эндпоинт внутренний (гейт `X-Internal-Key` BFF) и требует владения учеником (`OwnsStudent`), т.е. отправить может только аутентифицированный тренер своим адресам — не поверхность массового бомбинга. Усиление при появлении в ApiService общего лимитера — политика `email-send` по владельцу/IP (как в IdP §2).
