...
### ⏳ MoveAsync Арены флашит стор на каждый ход (низкий приоритет)
**Симптом (измерено, нагрузочный тест «10к играющих»):** `ArenaTournamentGrain.MoveAsync` вызывает
`FlushAsync` (Snapshot+WriteState всего состояния, O(N)) на **каждый** ход, хотя `_games` в персист вообще
не входят (Snapshot пишет только Players/мету) → мид-партийный flush почти вхолостую. На 5000 партий в
одном турнире грейн держит ~7 000 ход/с (p99 149 мс под max-прокачкой). **Рычаг:** коалесить как
регистрацию — писать стор только на финише партии (меняются очки/таблица) или по таймеру тика; ход сам в
персист не идёт. Снимет O(N)-на-ход и p99. **Приоритет низкий:** запас 7× над реалистичным спросом
(~1000 ход/с на 5000-партийный турнир, §2) достаточен; чинить при появлении турниров с активной игрой
существенно плотнее целевого темпа. См. [CAPACITY_PLANNING §8.1](CAPACITY_PLANNING.md).
