...
async Task StormScenario(TestCluster c, int target)
{
    ...
    Console.WriteLine("Вывод: регистрация коалесит запись стора (персист ведёт таймер) → ~O(1)/op, десятки тыс/с;");
    Console.WriteLine("       до фикса writing-on-join делал бёрст O(N²) (~100 ops/с при N=5000). Тик — дёшев (sub-ms).\n");
}
